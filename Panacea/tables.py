...
def get_farebox_and_vp_revenues(years, user_org_id, classification):
    ...
    farebox_modes = list(service_offered.objects.filter(organization_id__in=user_org_id).values_list('transit_mode_id', flat=True).distinct())
...
def build_expense_table(years, user_org_id, classification):
    ...
    data_expense = expense.objects.filter(organization_id__in = user_org_id, year__in = years, reported_value__isnull= False).values('expense_source_id__name', 'year').annotate(reported_value = Sum('reported_value'))
    fund_balance = fund_balance.objects.filter(organization_id__in = user_org_id, year__in = years, reported_value__isnull=False).values('fund_balance_type_id__name', 'year').annotate(reported_value = Sum('reported_value'))
    df_expense = read_frame(data_expense)
    fund_balance = read_frame(fund_balance)
    df_expense = df_expense.rename(columns ={'expense_source_id__name': 'revenue_source'})
    fund_balance = fund_balance.rename(columns = {'fund_balance_type_id__name': 'revenue_source'})
    df = pd.concat([fund_balance, df_expense], axis = 0)
...
def call_stylesheets(classification, type):
    ...
    elif classification == 'Transit' and type == 'expense':
        return list(stylesheets.objects.filter(transit_expense__isnull=False).values_list('transit_expense', flat=True))
...
def build_operations_data_table(years, org_id, classification):
    ...
    transitdata = transit_data.objects.filter(organization_id__in=org_id, year__in=years).values('transit_mode__rollup_mode', 'year', 'transit_metric').annotate(reported_value = Sum('reported_value'))
    count = 0
    ...
    if list(services_offered) == []:
        return pd.DataFrame()
    services_offered = ['Fixed Route', 'Commuter Rail', 'Light Rail', 'Route Deviated', 'Demand Response', 'Vanpool']
    for service in services_offered:
        if service == 'all':
            filter  = transit_data.objects.filter(organization_id__in = org_id, year__in = years).values('year', 'transit_metric__name').annotate(reported_value = Sum('reported_value')).order_by('order_in_summary')
        else:
            filter = transitdata.filter(transit_mode__rollup_mode=service)
        ...
        if df.empty == True:
            ...
            if str(classification) == 'Ferry':
                ...
                if service == 'all':
                    ...
                    mode_name = '{} Services'.format(service)
            else:
                mode_name = '{} Services'.format(service)
